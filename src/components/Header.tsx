  ...
  return (
    <>
      <header
        ...
        } ${isScrolled || !isHomePage ? "py-2.5" : "py-6"}`}
      >
        <div className="container mx-auto px-5 sm:px-6 lg:px-8">
          <nav className="flex items-center justify-between">
            ...
            <button
              ...
              className={`md:hidden relative p-2 transition-all duration-300 z-20 ${
            ...
            >
              <div className="relative w-7 h-7 flex items-center justify-center">
                {isMenuOpen ? (
                  <X size={28} className="absolute transition-all duration-300 rotate-90 scale-110" />
                ) : (
          ...
          {/* Unique Borderless Mobile Navigation */}
          {isMenuOpen && (
            <div 
              className={`md:hidden mt-6 transition-all duration-300 pb-24 ${
                isHomePage
            ...
            >
              ...
              {isHomePage && (
                <div className="mb-6 h-px bg-gradient-to-r from-transparent via-white/40 to-transparent"></div>
              )}
                ...
                {navItems.map((item, index) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {location.pathname === item.href && (
                        <span className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 rounded-r-full transition-all duration-500 ${
                          isHomePage && isMenuOpen 
                      ...
                      {location.pathname === item.href && isHomePage && isMenuOpen && (
                        <span className="absolute right-6 top-1/2 -translate-y-1/2 text-white/60 text-xs animate-pulse">
                          ●
                        </span>
                      )}
                    </Link>
              ...
              {isHomePage && (
                <div className="mt-6 h-px bg-gradient-to-r from-transparent via-white/40 to-transparent"></div>
              )}
            </div>
          )}
        </div>
        ...
        {isMenuOpen && isHomePage && (
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute top-0 left-1/4 w-64 h-64 bg-white/5 rounded-full blur-3xl animate-pulse-slow"></div>
            <div className="absolute bottom-0 right-1/4 w-64 h-64 bg-emerald-400/5 rounded-full blur-3xl animate-pulse-slow" style={{ animationDelay: '1s' }}></div>
          </div>
        )}
      </header>

      {/* Fixed Bottom Buttons for Mobile Only */}
      <div className="fixed bottom-0 left-0 right-0 md:hidden bg-gradient-to-t from-white via-white to-white/95 backdrop-blur-sm p-4 z-50 safe-bottom border-t border-gray-100">
        <div className="container mx-auto px-5 sm:px-6">
          <div className="flex gap-3">
            <a
              href="tel:+918680050504"
              className="flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-full font-semibold text-sm transition-all bg-white border-2 border-emerald-600 text-emerald-700 hover:bg-gradient-to-r hover:from-emerald-600 hover:to-emerald-700 hover:text-white shadow-lg hover:shadow-xl active:scale-95"
            >
              <Phone className="w-4 h-4" />
              Call
            </a>
